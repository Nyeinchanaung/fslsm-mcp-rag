...
from experiments.exp3_mcp_runtime.config import AGENTS_PATH, PASSIVE_R2_LOG_PATH, R2B_QUESTIONS_PATH
...
if __name__ == "__main__":
    ...
    questions = json.loads(R2B_QUESTIONS_PATH.read_text())
        ...
        for agent in agents:
            for condition in (Condition.S0, Condition.S1A, Condition.S1B):
                runner.run_session(
                    ...
                    condition=condition,
                    corpus_backed=question["question_type"] not in {"search"},
                    log_passive=True,
                )
    print("R2b full run complete.")
