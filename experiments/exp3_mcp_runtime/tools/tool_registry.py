...
`name` is the human-readable display label used in thesis text and UI.
`mcp_name` is the MCP-safe tool identifier used for protocol registration.
"""
...
def _to_mcp_name(label: str) -> str:
...
class MCPTool:
    ...
    token_cost: int
    mcp_name: str = field(init=False)

    def __post_init__(self) -> None:
        self.mcp_name = _to_mcp_name(self.name)
...
TOOL_BY_MCP_NAME: Dict[str, MCPTool] = {t.mcp_name: t for t in TOOL_REGISTRY}
...
def get_tool_by_mcp_name(tool_name: str) -> MCPTool:
    return TOOL_BY_MCP_NAME[tool_name]
...
def registry_summary() -> None:
    print(f"\n{'#':>3}  {'Display Name':<32} {'MCP Name':<28} {'Tokens':>6}")
    print("─" * 80)
    for t in TOOL_REGISTRY:
        print(f"{t.tool_id:>3}  {t.name:<32} {t.mcp_name:<28} {t.token_cost:>6}")
    print("─" * 80)
    print(f"     {'S0 total':<61} {s0_prompt_tokens():>6}")
