...
\begin{shadedquotation}
    ...
    \begin{enumerate}
        ...
        \item CLEAR PRESENTATION: Structure the answer clearly and coherently, but do not apply any learner-specific style adaptation.
    \end{enumerate}
\end{shadedquotation}
...
\subsection{Experiment 2 Hybrid Reranker Formula}
The Experiment 2 retrieval pipeline uses a hybrid reranking step only in the personalized R1 condition. After hybrid BM25+FAISS retrieval and reciprocal rank fusion (RRF), each candidate chunk $c_i$ receives a reranking score:
...
where $\alpha = 0.75$, so semantic relevance remains the dominant signal. The semantic term $\hat{s}_{\mathrm{sem}}(c_i)$ is not a fresh cosine similarity computed against the original query. Instead, it is the candidate's fused RRF retrieval score, normalized to the interval $[0,1]$ across the current candidate set:
...
The style term is a heuristic content-tag alignment score derived from the learner profile's reranking preferences:
...
where $h_i^{+}$ is the number of matched bias tags, $h_i^{-}$ is the number of matched deprioritized tags, and $M$ is the number of active bias tags for the current profile. This maps the net tag-alignment signal to $[0,1]$. In implementation terms, the reranker therefore combines normalized retrieval relevance with profile-conditioned heuristic tag alignment, rather than combining two embedding cosine similarities.
